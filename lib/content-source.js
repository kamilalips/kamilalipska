...
function isUnifiedPublished(value = "") {
...
async function fetchNotionPosts() {
  ...
  const filtered = results.filter((page) => {
    const unifiedStatus = notionToText(getNotionProperty(page, "Unified Status"));
    const destination =
      notionToText(getNotionProperty(page, "Destination website")) ||
      notionToText(getNotionProperty(page, "Destination Website")) ||
      notionToText(getNotionProperty(page, "Website"));

    const isApproved = isUnifiedPublished(unifiedStatus);
    return (
      isApproved &&
  ...
  return filtered
    .map((page) => {
      ...
      const metaTitle =
        notionToText(getNotionProperty(page, "Meta Title")) ||
        notionToText(getNotionProperty(page, "SEO Title")) ||
        notionToText(getNotionProperty(page, "Title"));
      const metaDescription =
        notionToText(getNotionProperty(page, "Meta Description")) ||
        notionToText(getNotionProperty(page, "SEO Description")) ||
        llmsDescription ||
        excerpt;
...
async function readLocalInsightsPosts() {
  let raw = null;
  // Prefer filesystem first (includeFiles on Vercel) to avoid a second serverless call to /api/insights-posts.
  try {
    const path = join(process.cwd(), "insights", "posts.json");
    raw = await readFile(path, "utf-8");
  } catch {
    // File missing (e.g. no includeFiles); fallback to fetch.
    const baseUrl = getBaseUrl();
    const url = `${baseUrl}/api/insights-posts`;
    try {
  ...
  return posts
    .filter((post) => {
      ...
      return isUnifiedPublished(status);
    })
    .map((post) => {
      ...
      const llmsDesc = post.llmsDescription ? stripHtml(post.llmsDescription) : excerpt;
      const metaTitle =
        stripHtml(post.metaTitle || post.meta_title || "") || title;
      const metaDescription =
        stripHtml(post.metaDescription || post.meta_description || "") || llmsDesc || excerpt;

      return {
        slug,
        title,
        excerpt,
        llmsDescription: llmsDesc,
        metaTitle,
        metaDescription,
...
/** Fast path: get one post by slug. Tries local file first to avoid Notion/WordPress when possible. */
